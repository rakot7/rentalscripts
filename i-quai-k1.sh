...
tar -xf rigel-1.22.1-linux.tar.gz
cd rigel-1.22.1-linux
rm quai.sh
cat <<EOF > quai.sh
#!/bin/bash
...
echo "command=/bin/bash -c 'cd /root/quai/rigel-1.22.1-linux && screen -dmS quai ./quai.sh && sleep infinity'" >> /etc/supervisor/conf.d/supervisord.conf
